...
namespace SharpLua.Parser
{
    public partial class Parser
    {
        ...
        private string MatchTerminalStringWithNextCharCheck(string terminalString, out bool success)
        {
            int currrent_position = position;
            foreach (char terminal in terminalString)
            {
            ...
            // Need extra check of the next char to see if the previous word is ended.
            if (Input.HasInput(position))
            {
                char nextChar = Input.GetInputSymbol(position);
